...
publish package-a 1 || test_failed $LINENO

# package-b, depends on package-a
echo "hello" > "$ROOT/it-s-a-me"

publish package-b 1 --requires package-a=1 \
    || test_failed $LINENO

# package-c
echo "C" > "$ROOT/see"

publish package-c 1 --requires package-a=1 \
    || test_failed $LINENO

# package-d, depends on package-c and package-b
echo "d" > "$ROOT/some_file"

publish package-d 1 --requires package-c=1 package-b=1 \
    || test_failed $LINENO
