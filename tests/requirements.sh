...
echo 20 > "$ROOT/file"
mkdir "$ROOT/dir"
echo 30 > "$ROOT/dir/file"

archive_and_upload package-a 2 || test_failed $LINENO

# package-b, depends on package-a
echo "hello" > "$ROOT/it-s-a-me"

archive_and_upload package-b 1 --requires package-a=2 \
    || test_failed $LINENO

# package-c
echo "C" > "$ROOT/see"

archive_and_upload package-c 1 || test_failed $LINENO
...
check_installed package-a 2 || test_failed $LINENO

# Files from package-a.
! [ -f "$TARGET/mine" ] || test_failed $LINENO
diff --brief "$TARGET/file" <(echo 20) > /dev/null || test_failed $LINENO
diff --brief "$TARGET/dir/file" <(echo 30) > /dev/null || test_failed $LINENO
